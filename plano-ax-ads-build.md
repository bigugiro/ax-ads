...
**Sprint 10 FECHADO em 2026-07-08.** Entregue White-label & hardening (Seção 8), escopo alinhado com o usuário em 3 decisões (white-label básico sem domínio customizado; admin mínimo via API sem tela dedicada... **acabou ganhando tela mesmo assim**, pra fechar o DoD "marca do cliente" com prova visual completa; LGPD = exclusão de conta + consentimento no signup, sem tocar schema de leads). **White-label básico**: `agencias.marca_nome/marca_cor/marca_logo_url` (migration 0018), `PATCH /agencias/marca` (RLS já exigia owner desde o Sprint 0 — `agencias_update` — mais `requireAcao('gerenciar_agencia')` como defesa em profundidade), `GET /me` como fonte única de "quem sou eu" pro front. `AppShell` aplica a cor da agência como override da CSS var `--brand` (conversão hex→"R G B", formato que os tokens da marca Dispara já usam) e troca o wordmark por nome/logo customizado — funciona em qualquer tela, não só na de configuração. **Admin mínimo**: `usuarios.super_admin` (migration 0019, nunca setável via API — só SQL direto do operador), `GET/PATCH /admin/agencias` cross-tenant atrás de `requireSuperAdmin`, com tela `/mais/admin` (link só aparece pra quem já é super_admin). **LGPD**: `POST /auth/signup` agora exige `aceite_termos` (`z.literal(true)`, vira parte do audit_log do signup — não é decorativo); `DELETE /agencias/me` (owner) apaga a agência inteira em cascata + os usuários correspondentes no Supabase Auth — irreversível, testado de ponta a ponta. **Perf/PWA**: todas as páginas viraram `React.lazy` + `Suspense` — build confirma cada rota como chunk próprio (2–13 kB) separado do bundle principal, resolvendo a dívida de bundle monolítico registrada desde o Sprint 0. **Pen-test básico**: rate limiter em memória, puro e testável (`permitirChamada`), aplicado em `/auth/signup` (5/min) e `/billing/webhook` (30/min); `npm audit` = 0 vulnerabilidades em produção (6 só em devDependencies do Vitest/esbuild, documentadas e aceitas por ora); `SECURITY_CHECKLIST.md` novo, consolidando as garantias de todas as sprints anteriores. 228 testes verdes (era 215).

- **Bug real pego no gate de integração (não em teste unitário isolado):** montar `adminRouter` com `app.use(adminRouter)` **sem prefixo de path**, tendo `requireSuperAdmin` no nível do router (`adminRouter.use(authenticate, requireSuperAdmin)`), fez o Express rodar esse gate pra **toda requisição do app**, não só `/admin/*` — Express roteia `router.use()` sem path pra qualquer request que atravesse o router, mesmo que ela não bata em nenhuma rota definida ali dentro. Resultado: 44 testes de sprints anteriores (Studio IA, Imagem) passaram a devolver 403 pra usuários legítimos. Corrigido montando com prefixo (`app.use('/admin', adminRouter)`, rotas internas relativas) — regra gravada no `SECURITY_CHECKLIST.md`: todo router com gate "duro" precisa de prefixo, nunca solto na raiz do app. `agenciasRouter` recebeu o mesmo tratamento por consistência, mesmo só tendo `authenticate` (que sozinho não rejeita ninguém, mas ainda rodaria à toa em rotas sem JWT como `/cron/*`).
- **Verificação desta vez, de novo, só a nível de API** (Playwright indisponível nesta sessão, como no Sprint 9) — confirmado end-to-end via HTTP real: `PATCH /agencias/marca` persiste e `GET /me` reflete, `GET /admin/agencias` corretamente nega usuário comum (403) e aceita super_admin promovido via SQL direto (200, lista cross-tenant). **Não verificado visualmente**: o header realmente trocando de "Dispara" laranja pra marca customizada, e os links condicionais (Marca/Admin) aparecendo/sumindo na aba Mais — conferir na próxima sessão com navegador disponível.

**Próximo passo:** Sprint 11 — Integração Meta (real): OAuth + cofre de tokens criptografado + `MetaProvider` na mesma interface do `AdsProvider`. **Primeiro canal real do produto** — inicia a burocracia de Business Verification/App Review (Seção 12 do plano, ~6 semanas de lead time) em paralelo à implementação.
